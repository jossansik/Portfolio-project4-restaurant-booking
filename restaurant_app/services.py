...
from restaurant_app.models import (
    MAX_HOUR, MIN_HOUR,
    Menu, MenuItem, Reservation, Table)
...
def make_reservation(user, table_id, start_date, num_guests, guest_fullname):
    ...
    existing_reservation = Reservation.objects.filter(
        table=table, reserved_start_date__gte=start_date,
        reserved_start_date__lt=end_date)
    ...
    reservation = Reservation.objects.create(
        ...
        reserved_start_date=start_date, guest_fullname=guest_fullname)
...
def get_timeslots(num_guests, date):
    ...
    for table in tables:
        existing_reservations = Reservation.objects.filter(
            table=table,
            reserved_start_date__gte=start_date,
            reserved_start_date__lte=end_date)
        ...
        for hour in range(MIN_HOUR, MAX_HOUR):
            ...
            if any(x.reserved_start_date.hour == hour
    ...
    for hour in range(MIN_HOUR, MAX_HOUR):
        ...
        is_reserved = all(
            v.reservedTable.is_reserved for
            v in reserved_table_timeslots_for_hour)
